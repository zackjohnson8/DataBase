...
// return 0 success
// else...
int lineHandler(const std::string& inputLine,
              ...
              )
{
  ...
  // Handle for no spaces
  if(holdInt != -1)
  {
    // Take in the first word and decide what kind of statement this is
    ...
    if(parsedWord == "CREATE")
    {
      ...
      // Create either DATABASE or TABLE
    ...
    if(parsedWord == "USE")
    {
      ...
      if(foundDup)
      {
        ...
        if(!holdTables->empty())
        {
          newCombo = new DataBaseTableCombo();
          newCombo->dataBase = holdDataBases->at(holdIndex);
          for(int index = 0; index < holdTables->size(); index++)
          {
